...
public class Filter implements Serializable, Queryable {
	...
	public Filter(Class<?> target, Condition condition, boolean negative, String value, String searchCriteria) {
		this(FilterTarget.findByClass(target), Lists.newArrayList(new FilterCondition(condition, negative, value, searchCriteria)));
	}
	...
	public Map<ConditionType, org.jooq.Condition> toCondition() {
		...
		for (ConvertibleCondition filterCondition : filterConditions) {
			filterCondition.toCondition(this.target)
					.forEach((key, value) -> addTransformedCondition(resultedConditions, filterCondition.getOperator(), value, key));
		}
