...
public class ProjectConfiguration implements Serializable {
	...
	public ProjectConfiguration(Set<ProjectAttribute> projectAttributes) {
		if (CollectionUtils.isNotEmpty(projectAttributes)) {
			Map<String, String> attributes = projectAttributes.stream()
					.collect(Collectors.toMap(projectAttribute -> projectAttribute.getAttribute().getName(), ProjectAttribute::getValue));
			...
			this.analyzerConfig = new ProjectAnalyzerConfig.ProjectAnalyzerConfigBuilder().minDocFreq(NumberUtils.toInt(attributes.get(
					MIN_DOC_FREQ.getValue()), ProjectAnalyzerConfig.MIN_DOC_FREQ))
					...
					.numberOfLogLines(NumberUtils.toInt(
							attributes.get(NUMBER_OF_LOG_LINES.getValue()),
							ProjectAnalyzerConfig.NUMBER_OF_LOG_LINES
					))
					...
					.build();
			this.projectEmailConfig = new ProjectEmailConfig.ProjectEmailConfigBuilder().emailEnabled(BooleanUtils.toBooleanObject(
					attributes.get(EMAIL_ENABLED.getValue()))).from(attributes.get(EMAIL_FROM.getValue())).build();
		}
	}
