...
import com.epam.ta.reportportal.grabage.analyzer.ILogIndexer;
...
import static com.epam.ta.reportportal.database.entity.enums.StatusEnum.IN_PROGRESS;
...
//				projectName, itemRepository.selectIdsNotInIssueByLaunch(launchId, TestItemIssueType.TO_INVESTIGATE.getLocator()));
	...
	private void validate(Launch launch, Users user, Project project) {
		expect(launch.getProjectId(), equalTo(project.getName())).verify(FORBIDDEN_OPERATION,
				formattedSupplier("Target launch '{}' not under specified project '{}'", launch.getId(), project.getName())
		);

		expect(launch, not(l -> l.getStatus().equals(IN_PROGRESS))).verify(LAUNCH_IS_NOT_FINISHED,
