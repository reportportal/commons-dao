...
public class HealthCheckTableChain implements WidgetProviderChain<HealthCheckTableGetParams, List<HealthCheckTableContent>> {
	...
	public List<HealthCheckTableContent> apply(HealthCheckTableGetParams params) {
		...
		boolean contentFirst =
				healthCheckTableStatisticsChain.resolvePriority(params) - healthCheckTableColumnChain.resolvePriority(params) >= 0;
		...
		return params.getSort().get().filter(order -> ALLOWED_SORTING.contains(order.getProperty())).findFirst().map(it -> {
			Comparator<HealthCheckTableContent> comparator = Comparator.comparingDouble(HealthCheckTableContent::getPassingRate);
			return result.stream().sorted(it.isAscending() ? comparator : comparator.reversed()).collect(toList());
		}).orElse(result);
	}

	private List<HealthCheckTableContent> concat(boolean contentFirst, Map<String, HealthCheckTableStatisticsContent> content,
		...
		if (contentFirst) {
			...
			columnMapping.forEach((key, attributes) -> {
				HealthCheckTableContent resultEntry = ofNullable(content.remove(key)).map(statisticsContent -> entryFromStatistics(key,
						statisticsContent
