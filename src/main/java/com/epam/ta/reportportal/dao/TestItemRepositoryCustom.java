...
public interface TestItemRepositoryCustom extends FilterableRepository<TestItem> {
   ...
   *               {@link com.epam.ta.reportportal.commons.querygen.FilterTarget#TEST_ITEM_TARGET}
   ...
   *                         {@link com.epam.ta.reportportal.entity.launch.Launch} query
   ...
   * {@link com.epam.ta.reportportal.entity.project.Project} grouped by
   * {@link TestItem#getTestCaseHash()} and ordered by {@link TestItem#getStartTime()} `DESCENDING`
   * within group. Max group size equals to the provided `historyDepth` value.
   *
   ...
   * {@link com.epam.ta.reportportal.entity.project.Project} with provided `projectId` and
   * {@link Launch#getName()} equal to the provided `launchName` value. Result is grouped by
   * {@link TestItem#getTestCaseHash()} and is ordered by {@link TestItem#getStartTime()}
   * `DESCENDING` within group. Max group size equals to the provided `historyDepth` value.
   *
   * @param filter       {@link Queryable}
   * @param pageable     {@link Pageable}
   * @param projectId    {@link Project#getId()}
   ...
   * Loads items {@link TestItemHistory} - {@link TestItem} executions from the
   * {@link com.epam.ta.reportportal.entity.project.Project} with provided `projectId` and
   * {@link Launch} which IDs are in provided `launchIds`. Result is grouped by
   * {@link TestItem#getTestCaseHash()} and is ordered by {@link TestItem#getStartTime()}
   * `DESCENDING` within group. Max group size equals to the provided `historyDepth` value.
   *
   * @param filter       {@link Queryable}
   ...
   * Loads items {@link TestItemHistory} - {@link TestItem} executions from the whole
   * {@link com.epam.ta.reportportal.entity.project.Project} grouped by
   * {@link TestItem#getTestCaseHash()} ordered by {@link TestItem#getStartTime()} `DESCENDING`
   * within group. Max group size equals to the provided `historyDepth` value. Items result query is
   * built from filters with
   * {@link com.epam.ta.reportportal.commons.querygen.FilterTarget#LAUNCH_TARGET} and
   * {@link com.epam.ta.reportportal.commons.querygen.FilterTarget#TEST_ITEM_TARGET}
   *
   * @param isLatest         Flag for retrieving only latest launches
   ...
   * {@link Launch#getName()} equal to the provided `launchName` value. Result is grouped by
   * {@link TestItem#getTestCaseHash()} and is ordered by {@link TestItem#getStartTime()}
   * `DESCENDING` within group. Max group size equals to the provided `historyDepth` value. Items
   * result query is built from filters with
   * {@link com.epam.ta.reportportal.commons.querygen.FilterTarget#LAUNCH_TARGET} and
   * {@link com.epam.ta.reportportal.commons.querygen.FilterTarget#TEST_ITEM_TARGET}
   *
   * @param isLatest         Flag for retrieving only latest launches
  ...
  /**
   * True if the {@link com.epam.ta.reportportal.entity.item.TestItem} with matching 'status' and
   * 'launchId' was started within the provided 'period'
   *
  ...
  /**
   * Select {@link PathName} containing ids and names of all items in a tree till current and launch
   * name and number for each item id from the provided collection
   *
   ...
   * {@link com.epam.ta.reportportal.entity.enums.LogLevel#ERROR_INT} excluding {@link TestItem}
   ...
   * @param excludedIssueTypes {@link Collection} of {@link IssueType#getId()} to exclude
   *                           {@link TestItem} with provided type id
   ...
   *
   * @param itemIds {@link Collection} of {@link TestItem#getItemId()} that should be filtered by
   *                having descendants
   * @return {@link List} of {@link TestItem#getItemId()}
   */
   ...
   * of the {@link com.epam.ta.reportportal.entity.log.Log} entities of all descendants for each
   * {@link NestedStep} and {@link NestedStep#hasContent} flag to check whether entity is a last one
   * in the descendants tree or there are {@link com.epam.ta.reportportal.entity.log.Log} or
   * {@link NestedStep} entities exist under it
   *
   * @param ids       {@link Collection} of the {@link TestItem#itemId}
   * @param logFilter {@link Queryable} with {@link com.epam.ta.reportportal.entity.log.Log} target,
   *                  to evaluate 'hasContent' flag and   attachments count
