...
public interface TestItemRepository extends ReportPortalRepository<TestItem, Long>, TestItemRepositoryCustom {
	 ...
	 */
	@Query(value = "SELECT exists(SELECT 1 FROM test_item item JOIN test_item_results result ON item.item_id = result.result_id WHERE item.parent_id = :parentId AND result.status <> cast(:#{#status.name()} AS STATUS_ENUM))", nativeQuery = true)
	...
	@Query(value = "SELECT exists(SELECT 1 FROM test_item item JOIN test_item_results result ON item.item_id = result.result_id WHERE item.parent_id <> :parentId AND result.status = cast(:#{#status.name()} AS STATUS_ENUM) AND item.path <@ cast(:path AS LTREE))", nativeQuery = true)
	boolean hasStatusByParent(@Param("parentId") Long parentId, @Param("path") String path, @Param("status") StatusEnum status);
