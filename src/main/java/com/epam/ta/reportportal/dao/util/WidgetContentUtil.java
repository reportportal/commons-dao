...
public class WidgetContentUtil {
  ...
  public static final Function<Result<? extends Record>, OverallStatisticsContent> OVERALL_STATISTICS_FETCHER = result -> {
    ...
    result.forEach(
        record -> ofNullable(record.get(STATISTICS_FIELD.NAME)).ifPresent(v -> values.put(v,
  ...
  public static final BiFunction<Result<? extends Record>, List<String>, List<LaunchesTableContent>> LAUNCHES_TABLE_FETCHER = (result, contentFields) -> {
    ...
    Optional<Field<?>> statisticsField = ofNullable(
        result.field(fieldName(STATISTICS_TABLE, SF_NAME)));
    Optional<Field<?>> startTimeField = ofNullable(
        result.field(LAUNCH.START_TIME.getQualifiedName().toString()));
    ...
    result.forEach(record -> {
      ...
      statisticsField.flatMap(sf -> ofNullable(record.get(sf, String.class)))
          .ifPresent(v -> content.getValues()
              .put(v, ofNullable(
                  record.get(fieldName(STATISTICS_TABLE, STATISTICS_COUNTER), String.class)).orElse(
                  "0")));
      ...
      itemAttributeIdField.flatMap(f -> ofNullable(record.get(f))).ifPresent(id -> {
        Set<ItemAttributeResource> attributes = ofNullable(content.getAttributes()).orElseGet(
  ...
  public static final Function<Result<? extends Record>, List<FlakyCasesTableContent>> FLAKY_CASES_TABLE_FETCHER = result -> result.stream()
      .map(record -> {
        ...
        entry.setStartTime(Collections.singletonList(
            record.get(DSL.field(fieldName(START_TIME_HISTORY)), Date.class)));
        return entry;
      })
      .collect(Collectors.toList());
  ...
  public static final Function<Result<? extends Record>, List<ChartStatisticsContent>> INVESTIGATED_STATISTICS_FETCHER = result -> {
    List<ChartStatisticsContent> statisticsContents = Lists.newArrayListWithExpectedSize(
        result.size());
    result.forEach(
        r -> ofNullable(r.get(TO_INVESTIGATE, Double.class)).ifPresent(toInvestigatePercentage -> {
  ...
  public static final RecordMapper<? super Record, NotPassedCasesContent> NOT_PASSED_CASES_CONTENT_RECORD_MAPPER = r -> {
    ...
    res.setValues(Collections.singletonMap(NOT_PASSED_STATISTICS_KEY,
  ...
  private static final Function<Result<? extends Record>, Map<Long, ChartStatisticsContent>> STATISTICS_FETCHER = result -> {
    ...
    result.forEach(record -> {
      ...
      ofNullable(record.get(fieldName(STATISTICS_TABLE, SF_NAME), String.class)).ifPresent(
          v -> content.getValues()
              .put(v, ofNullable(
                  record.get(fieldName(STATISTICS_TABLE, STATISTICS_COUNTER), String.class)).orElse(
                  "0")));
  ...
  public static final Function<Result<? extends Record>, List<ChartStatisticsContent>> BUG_TREND_STATISTICS_FETCHER = result -> {
    ...
    resultMap.values()
        .forEach(content -> content.getValues()
            .put(TOTAL, String.valueOf(
                content.getValues().values().stream().mapToInt(Integer::parseInt).sum())));
  ...
  public static final BiFunction<Result<? extends Record>, Map<String, String>, Map<String, List<ProductStatusStatisticsContent>>> PRODUCT_STATUS_FILTER_GROUPED_FETCHER = (result, attributes) -> {
    ...
    Optional<? extends Field<?>> attributeField = ofNullable(
        result.field(fieldName(ATTR_TABLE, ATTRIBUTE_VALUE)));
    Optional<Field<?>> startTimeField = ofNullable(
        result.field(LAUNCH.START_TIME.getQualifiedName().toString()));
    Optional<Field<?>> statusField = ofNullable(
        result.field(LAUNCH.STATUS.getQualifiedName().toString()));
    ...
    result.forEach(record -> {
      ...
      ProductStatusStatisticsContent content = PRODUCT_STATUS_WITHOUT_ATTRIBUTES_MAPPER.apply(
          productStatusMapping, record);
      startTimeField.ifPresent(f -> content.setStartTime(record.get(f, Timestamp.class)));
      ...
      if (attributeField.isPresent()) {
        ...
        attributes.entrySet()
            ...
            .forEach(
                attribute -> proceedProductStatusAttributes(record, attribute.getKey(), content));
    ...
    return filterMapping.entrySet()
        ...
        .collect(LinkedHashMap::new,
            (res, filterMap) -> res.put(filterMap.getKey(),
                new ArrayList<>(filterMap.getValue().values())),
  ...
  private static void proceedProductStatusAttributes(Record record, String columnName,
    ...
    ofNullable(record.get(fieldName(ATTR_TABLE, ATTRIBUTE_VALUE), String.class)).ifPresent(
        value -> {
          Map<String, Set<String>> attributesMapping = ofNullable(
              content.getAttributes()).orElseGet(LinkedHashMap::new);
