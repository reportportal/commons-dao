...
public class HealthCheckTableStatisticsChain
  ...
  /**
   * If sorting order is {@link Sort.Order#isAscending()} and statistics criteria doesn't exist in
   * the result content items without statistics will be put after items with existing criteria. If
   * there is no statistics criteria in the result content it means statistics counter = 0 so items
   * should be put to the top
   *
   * @param params {@link HealthCheckTableGetParams}
   * @param result {@link Map} with 'attribute value' as key and
   *               {@link HealthCheckTableStatisticsContent} as value
