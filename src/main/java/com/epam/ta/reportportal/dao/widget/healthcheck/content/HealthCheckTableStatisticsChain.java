...
public class HealthCheckTableStatisticsChain
	...
	public Map<String, HealthCheckTableStatisticsContent> apply(HealthCheckTableGetParams params) {
		Map<String, HealthCheckTableStatisticsContent> result = statisticsQueryProvider.andThen(healthCheckTableStatisticsProvider)
				.apply(params);
	 ...
	 * items without statistics will be put after items with existing criteria.
	 * If there is no statistics criteria in the result content it means statistics counter = 0 so items should be put to the top
	 *
	...
	private Map<String, HealthCheckTableStatisticsContent> getResult(HealthCheckTableGetParams params,
		...
		return ofNullable(params.getSort().getOrderFor(EXECUTIONS_FAILED)).filter(Sort.Order::isAscending).map(sorting -> {
			Map<String, HealthCheckTableStatisticsContent> resortedResult = result.entrySet()
					...
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (prev, curr) -> curr, LinkedHashMap::new));
