...
public interface LaunchRepository extends ReportPortalRepository<Launch, Long>,
  ...
  /**
   * Finds launch by {@link Launch#getUuid()} and sets a lock on the found launch row in the
   * database. Required for fetching launch from the concurrent environment to provide
   * synchronization between dependant entities
   *
  ...
  List<Launch> findByProjectIdAndStartTimeGreaterThanAndMode(Long projectId, LocalDateTime after,
      LaunchModeEnum mode);
  ...
  List<Long> findIdsByProjectIdAndStartTimeBefore(@Param("projectId") Long projectId,
      @Param("before") LocalDateTime before, @Param("size") int limit);
  ...
  List<Long> findIdsByProjectIdAndStartTimeBefore(@Param("projectId") Long projectId,
      @Param("before") LocalDateTime before, @Param("pageSize") int limit,
  ...
  /**
   * Finds the latest(that has max {@link Launch#number} {@link Launch} with specified {@code name}
   * and {@code projectId}
   *
