...
public class UserFilterRepositoryCustomImpl implements UserFilterRepositoryCustom {
	...
	@Override
	public Page<UserFilter> getPermittedFilters(Long projectId, Filter filter,
		...
		return
			PageableExecutionUtils.getPage(
				USER_FILTER_FETCHER.apply(dsl.fetch(QueryBuilder.newBuilder(filter)
					...
					.addCondition(JAclEntry.ACL_ENTRY.SID.in(dsl
							.select(JAclSid.ACL_SID.ID)
	...
	@Override
	public Page<UserFilter> getOwnFilters(Long projectId, Filter filter, Pageable pageable,
        String userName) {
		return
			PageableExecutionUtils.getPage(
				USER_FILTER_FETCHER.apply(dsl.fetch(QueryBuilder.newBuilder(filter)
					.addCondition(JAclObjectIdentity.ACL_OBJECT_IDENTITY.OWNER_SID.in(dsl
						.select(JAclSid.ACL_SID.ID)
						.from(JAclSid.ACL_SID)
						.where(JAclSid.ACL_SID.SID.eq(userName))))
                    .addCondition(JFilter.FILTER.PROJECT_ID.eq(projectId))
	...
	@Override
	public Page<UserFilter> getSharedFilters(Long projectId, Filter filter, Pageable pageable,
        String userName) {
		return
			PageableExecutionUtils.getPage(
				USER_FILTER_FETCHER.apply(dsl.fetch(QueryBuilder.newBuilder(filter)
					...
					.addCondition(JAclEntry.ACL_ENTRY.SID.in(dsl
						...
						.from(JAclSid.ACL_SID)
						.where(JAclSid.ACL_SID.SID.eq(userName))))
					.addCondition(JAclObjectIdentity.ACL_OBJECT_IDENTITY.OWNER_SID.notIn(dsl
						.select(JAclSid.ACL_SID.ID)
						.from(JAclSid.ACL_SID)
						.where(JAclSid.ACL_SID.SID.eq(userName))))
					.addCondition(JFilter.FILTER.PROJECT_ID.eq(projectId))
