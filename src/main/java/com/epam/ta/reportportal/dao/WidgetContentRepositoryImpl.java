...
public class WidgetContentRepositoryImpl implements WidgetContentRepository {
	...
	public List<CumulativeTrendChartEntry> cumulativeTrendChart(String viewName, String levelAttributeKey, String parentAttribute) {
		final SelectOnConditionStep<? extends Record4> baseQuery = dsl.select(fieldName(ID),
		...
		if (parentAttribute != null) {
			...
			Table<?> unnestedArray = table(select(field(sql("unnest(?)", max(fieldName("ids")))).as(ID)).from(viewName)
					...
					.and(field(viewName, ATTRIBUTE_VALUE).cast(String.class).eq(split[1]))).as(UNNESTED_ARRAY);
