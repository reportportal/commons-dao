...
public interface ProjectRepository extends ReportPortalRepository<Project, Long>,
  ...
  boolean existsByKey(String key);@Query(value = "SELECT p.* FROM project p JOIN project_user pu on p.id = pu.project_id JOIN users u on pu.user_id = u.id WHERE u.login = :login", nativeQuery = true)
