...
public class QueryBuilder {
	...
	public QueryBuilder with(Filter filter) {
		...
		for (FilterCondition filterCondition : filter.getFilterConditions()) {
			...
			ComplexSearchCriteria filterCriteria = filterSearchCriteriaPreProcessor(filterCondition.getSearchCriteria());
			Optional<CriteriaHolder> criteriaHolder = map.getCriteriaHolderUnchecked(filterCriteria.getGlobalSearchCriteria());
			...
			if (criteriaHolder.get().isReference()) {
				...
				if (criteriaHolder.get().getQueryCriteria().contains(IssueCounter.DEFECTS_FOR_DB)) {
					String rebuilded = criteriaRebuilder(criteriaHolder.get().getQueryCriteria(), filterCriteria.getExtension());
	 ...
	 *            - initial searchCriteria parameter from FilterCondition object
