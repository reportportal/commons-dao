...
public class DataStoreConfiguration {
   ...
   * forced to AWS Signature Version 4 ({@link S3Constants#PROPERTY_SIGNER_VERSION}) because SigV2
   * canonicalizes the URL-encoded path (e.g. {@code /bucket/Azure%20DevOps}), while SeaweedFS
   * verifies the signature against the decoded path ({@code /bucket/Azure DevOps}), causing a
   * {@code SignatureDoesNotMatch} → 403 → {@code ProtocolException: Server rejected operation}
   * during the {@code Expect: 100-continue} preflight.
   *
   * @param accessKey access key
   * @param secretKey secret key
   * @param endpoint  SeaweedFS S3 gateway endpoint URL
   * @param region    region used for SigV4 credential scope (must match the backend expectation,
   *                  e.g. {@code us-east-1})
   * @return {@link BlobStore}
   */
  @Bean
