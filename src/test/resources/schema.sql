...
CREATE TABLE server_settings (
  id    BIGSERIAL CONSTRAINT server_settings_id PRIMARY KEY,
...
CREATE TABLE project_configuration (
  id                                BIGINT PRIMARY KEY REFERENCES project (id) ON DELETE CASCADE,
...
CREATE TABLE bug_tracking_system (
  id          BIGSERIAL CONSTRAINT bug_tracking_system_pk PRIMARY KEY AUTO_INCREMENT,
  url         VARCHAR                                          NOT NULL,
  type        VARCHAR                                          NOT NULL,
  bts_project VARCHAR                                          NOT NULL,
  project_id  BIGINT REFERENCES project (id) ON DELETE CASCADE DEFERRABLE,
...
(
  id                    BIGINT PRIMARY KEY REFERENCES integration (id) ON DELETE CASCADE,
  url                   VARCHAR(256),
  base_dn               VARCHAR(256),
...
CREATE TABLE filter (
  ...
  name        VARCHAR                        NOT NULL,
  project_id  BIGINT REFERENCES project (id) DEFERRABLE,
  target      VARCHAR                        NOT NULL,
  description VARCHAR
);

CREATE TABLE user_filter (
  id BIGINT NOT NULL REFERENCES filter (id)
);
...
CREATE TABLE test_item_results (
  result_id BIGINT PRIMARY KEY REFERENCES test_item_structure (structure_id) ON DELETE CASCADE,
  status    STATUS_ENUM NOT NULL,
  end_time  TIMESTAMP,
...
CREATE TABLE test_item (
  item_id       BIGINT PRIMARY KEY REFERENCES test_item_structure (structure_id) ON DELETE CASCADE,
  name          VARCHAR(256),
...
CREATE TABLE issue_group (
  issue_group_id BIGSERIAL PRIMARY KEY,
  issue_group    VARCHAR(64) NOT NULL
);
...
CREATE TABLE issue (
  issue_id          BIGINT PRIMARY KEY REFERENCES test_item_results (result_id) ON DELETE CASCADE,
...
CREATE TABLE ticket (
  ...
  bts_id       INTEGER REFERENCES bug_tracking_system (id) ON DELETE CASCADE ,
